...
print('sin integral')
ds = a / (n*140)
s = (0.5 + np.arange(n*140)) * ds
v = (1/np.pi)*(np.sqrt(s)*np.arcsin(np.sqrt((1-a)/(1-s))))
print(v.sum()*ds, (np.sqrt(1-a)+0.5*a*np.sqrt(1-a)+a**1.5-1)/3)
print('scalar integral==<t-^2> (this converges slowly!)')
v = (1/np.pi)*(s**1.5 / (1-s))*np.sqrt((1-a)/(a-s))
print(v.sum()*ds, 1 - np.sqrt(1-a) * ((a+2)/2), (tm*tm).mean())
...
correl = (m2 - m1*tp_m1)/(0.5 *m1*(1-np.sqrt(a))*np.sqrt(np.sqrt((1-a)*a)))
